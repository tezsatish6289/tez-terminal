...
const ZONES_DOC = "config/nifty_zones";
...
const ZONE_KEYS: (keyof Omit<NiftyZones, "manualOverride" | "momentumLookbackMin">)[] = [
...
export async function loadNiftyPriceHistory(
 ...
 * Mirrors loadEffectiveHeatmapZones in heatmap-zones/route.ts.
 */
export async function loadEffectiveNiftyZones(
  db: import("firebase-admin/firestore").Firestore,
): Promise<{ zones: NiftyZones; autoZoneClearReason: string | null }> {
  let zones = parseNiftyZones({});
  let autoZoneClearReason: string | null = null;
    ...
    try {
      ...
      if (sugSnap.exists) {
        ...
        if (!isStale && hasZones && goodGap) {
          ...
          autoZoneClearReason = !hasZones ? "missing_suggested" : !goodGap ? "insufficient_gap" : "stale";
...
export async function PUT(request: NextRequest) {
  ...
  await db.doc(ZONES_DOC).set(existing);
