...
export default function BillingPage() {
  ...
  const daysLeft = getDaysRemaining(endDate ?? null);
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground">
      ...
      <div className="flex-1 w-full max-w-2xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="rounded-xl border border-white/[0.06] bg-gradient-to-b from-[#141416] to-[#0f0f11] p-6">
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className={cn("text-lg font-black", statusColor)}>{statusLabel}</span>
              </div>
              {endDate && (
          ...
          <Link
            href="/subscribe"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-accent text-accent-foreground font-bold text-sm hover:bg-accent/90 transition-colors"
          >
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link
              href="/subscribe"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-accent/10 border border-accent/20 text-accent text-[11px] font-bold uppercase tracking-wider hover:bg-accent/20 transition-colors"
            >
          ...
          {payments.length === 0 ? (
            ...
            <div className="space-y-2">
              {payments.map((payment) => {
                const cfg = STATUS_CONFIG[payment.status] || STATUS_CONFIG.waiting;
                ...
                return (
                  ...
                  >
                    <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-white/[0.04]")}>
