...
import { usePublicBots, type PublicBotApiRow } from "@/hooks/use-public-bots";
...
/**
 * Render either a coloured logo (when the registry provides one) or the
 * text fallback. Kept inline because the dashboard's visual rhythm is
 * tight and matches what the landing page does for the same set of bots.
 */
function BotIcon({ bot, size = 28 }: { bot: PublicBotApiRow; size?: number }) {
  if (bot.logo) {
    return (
      <Image
        src={bot.logo}
...
function NotConnected({ stats, onDeploy }: { stats: BotStats | null; onDeploy: () => void }) {
  ...
  // Two derived lists. `publicLive === true` is "Live" (currently only
  // Crypto Bot — drives the hero stats card). Everything else is
  // "Coming soon" and renders as a muted row beneath. The split mirrors
  // the landing page so the dashboard never advertises bots the catalog
  // hides, and vice versa.
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
      ...
      {/* All bots — live + coming soon. List comes from the same
          registry as the landing page / catalog, so this view never
          drifts. Currently only `crypto` is publicLive; the other four
          render as muted "Coming Soon" rows until an admin flips their
          publicLive flag (passphrase-gated). */}
      <div
        className="rounded-2xl overflow-hidden"
        style={{ border: "1px solid rgba(90,140,220,0.15)" }}
      >
        {/* Live bots — full stats card. Today this is just Crypto Bot;
            additional live bots would render with the same shape once
            we wire per-bot stats into `/api/freedombot/stats`. */}
        {liveBots.map((bot) => {
        ...
        {/* Coming-soon bots — every registered bot whose `publicLive`
            is currently false. Admin flips publicLive via the
            cockpit's passphrase dialog; this list reacts on the next
            page load. */}
...
function Connected({ deployment, deployments, stats, trades, aggregates, cumulativeByTradeId, tradesSyncing, onOpenSettings, onSelectDeployment, onRefreshTrade }: {
  ...
  // Tabs at the top of the dashboard come from the same public-bots
  // registry as the catalog. `bot.deployKey` is the canonical key on
  // each deployment doc (e.g. "CRYPTO", "BTC"), so the active-tab
  // comparison is a direct string match. `publicLive` drives the
  // "Soon" badge — no separate hardcoded list to drift from.
  const { bots: publicBots } = usePublicBots();
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-4">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
        {publicBots.map((bot) => {
          const isActive = bot.deployKey === deployment.bot;
          return (
            <div
              key={bot.id}
