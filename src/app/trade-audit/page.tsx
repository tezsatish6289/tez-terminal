...
function ChopOscillatorCard() {
  ...
  useEffect(() => {
    fetch(`/api/admin/chop-history?hours=${range}`)
      .then((r) => r.json())
      .then((d) => { setData(d.points || []); setLoaded(true); })
      .catch(() => setLoaded(true));
  }, [range]);

  const chartData = useMemo(() => {
    return data
      .filter((p) => p[selectedTf]?.ratio != null)
      .map((p) => {
        const d = new Date(p.timestamp);
        return {
          time: format(d, "HH:mm"),
          fullTime: format(d, "MMM dd HH:mm"),
          ratio: Math.round((p[selectedTf].ratio ?? 0) * 1000) / 1000,
          bullishKills: p[selectedTf].bullishKills ?? 0,
          bearishKills: p[selectedTf].bearishKills ?? 0,
          totalEvents: p[selectedTf].totalEvents ?? 0,
          isChoppy: p[selectedTf].isChoppy ?? false,
        };
      });
  }, [data, selectedTf]);

  const latestEntry = chartData.length > 0 ? chartData[chartData.length - 1] : null;
  const latestRatio = latestEntry?.ratio ?? null;

  return (
    <Popover>
      <PopoverTrigger asChild>
        <button className={cn(
          "flex flex-col items-center justify-center gap-1 px-3 lg:px-4 py-3 rounded-lg border transition-all hover:border-white/20 cursor-pointer",
          latestRatio != null && latestRatio > CHOP_THRESHOLD
            ? "border-rose-500/20 bg-rose-500/[0.05]"
            : "border-emerald-500/20 bg-emerald-500/[0.05]",
        )}>
          <span className="text-[8px] lg:text-[9px] font-bold uppercase tracking-widest text-muted-foreground/50">Chop</span>
          {latestRatio != null ? (
            <span className={cn(
              "text-base lg:text-lg font-black font-mono",
              latestRatio > CHOP_THRESHOLD ? "text-rose-400" : "text-emerald-400",
            )}>
              {(latestRatio * 100).toFixed(0)}%
            </span>
          ) : (
            <span className="text-base lg:text-lg font-black font-mono text-muted-foreground/30">—</span>
          )}
          <span className={cn(
            "text-[7px] font-bold uppercase tracking-widest",
            latestRatio != null && latestRatio > CHOP_THRESHOLD ? "text-rose-400/60" : "text-emerald-400/60",
          )}>
            {latestRatio != null ? (latestRatio > CHOP_THRESHOLD ? "Choppy" : "Trending") : "No Data"}
          </span>
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-[480px] p-0 border-white/10 bg-[#0e0e10]" align="end">
        <div className="p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xs font-black uppercase tracking-wider text-white">Chop Oscillator</h3>
              <p className="text-[9px] text-muted-foreground/50 mt-0.5">
                Ratio &gt; {(CHOP_THRESHOLD * 100).toFixed(0)}% = choppy → trades blocked
              </p>
            </div>
            <div className="flex gap-1">
              <PillGroup options={CHOP_TF_OPTIONS} value={selectedTf} onChange={setSelectedTf} />
            </div>
          </div>

          <div className="flex justify-end">
            <PillGroup options={CHOP_RANGE_OPTIONS} value={range} onChange={setRange} />
          </div>
          ...
          {!loaded ? (
            ...
            <div className="flex items-center justify-center h-40 text-muted-foreground/30 text-xs font-bold uppercase">
              Not enough data yet
            </div>
          ) : (
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                  ...
                  <ReTooltip
                    contentStyle={TOOLTIP_STYLE}
                    formatter={(value: number) => [`${(value * 100).toFixed(1)}%`, "Chop Ratio"]}
                  ...
                  <ReferenceLine
                    ...
                    strokeDasharray="6 3"
                    label={{ value: "Threshold", position: "right", fill: "rgba(244,63,94,0.5)", fontSize: 9 }}
                  />
                  <Line
                    type="monotone"
                    dataKey="ratio"
          ...
          {latestEntry && (
            <div className="flex items-center justify-between text-[9px] px-1 pt-1 border-t border-white/5">
              <span className="text-muted-foreground/40">
                Bull kills: <span className="text-emerald-400 font-bold">{latestEntry.bullishKills}</span>
                {" · "}Bear kills: <span className="text-rose-400 font-bold">{latestEntry.bearishKills}</span>
                {" · "}Events: <span className="text-white/50 font-bold">{latestEntry.totalEvents}</span>
              </span>
              <span className={cn(
                "font-black uppercase tracking-widest",
                latestEntry.isChoppy ? "text-rose-400" : "text-emerald-400",
              )}>
                {latestEntry.isChoppy ? "Choppy" : "Trending"}
              </span>
            </div>
          )}
        </div>
