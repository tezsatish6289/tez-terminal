...
import { LayoutDashboard, LineChart, History, Zap, ShieldCheck, Heart, TrendingUp, TrendingDown } from "lucide-react";
...
const navItems = [
  ...
  { name: "Signals", icon: Zap, active: false },
    ...
    <aside className="w-64 border-r bg-sidebar hidden lg:flex flex-col h-screen sticky top-0 overflow-hidden">
      ...
      <nav className="px-4 space-y-1">
        {navItems.map((item) => (
          ...
          >
            <item.icon className={cn("h-5 w-5", item.active ? "text-accent-foreground" : "group-hover:text-accent")} />
