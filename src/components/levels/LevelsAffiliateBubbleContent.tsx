"use client";

/** Gold-coin style content for the Refer & Earn map bubble. */
export function LevelsAffiliateBubbleContent({ compact }: { compact?: boolean }) {
  return (
    <div className="flex flex-col items-center justify-center px-2 text-center pointer-events-none select-none">
      <span
        aria-hidden
        className="relative mb-1 flex items-center justify-center rounded-full"
        style={{
          width: compact ? 28 : 36,
          height: compact ? 28 : 36,
          background:
            "radial-gradient(circle at 35% 30%, #fde68a 0%, #f59e0b 42%, #b45309 78%, #78350f 100%)",
          boxShadow:
            "0 0 14px rgba(245,158,11,0.55), inset 0 1px 2px rgba(255,255,255,0.45), inset 0 -2px 4px rgba(120,53,15,0.45)",
          border: "1.5px solid rgba(253,230,138,0.85)",
        }}
      >
        <span
          className="font-black leading-none"
          style={{
            fontSize: compact ? 13 : 16,
            color: "#78350f",
            textShadow: "0 1px 0 rgba(254,243,199,0.55)",
          }}
        >
          ₹
        </span>
      </span>
      <span
        className="font-black leading-none tracking-tight uppercase"
        style={{
          fontSize: compact ? 8 : 10,
          color: "#fef3c7",
          textShadow: "0 0 10px rgba(245,158,11,0.5)",
        }}
      >
        Refer & Earn
      </span>
      <span
        className="font-black leading-none mt-0.5 tracking-wide uppercase"
        style={{
          fontSize: compact ? 11 : 13,
          color: "#fde68a",
          textShadow: "0 0 12px rgba(251,191,36,0.65)",
        }}
      >
        CASH
      </span>
      <span
        className="font-semibold leading-none mt-1 uppercase tracking-wide"
        style={{ fontSize: compact ? 7 : 8, color: "rgba(254,243,199,0.8)" }}
      >
        up to 30%
      </span>
    </div>
  );
}
