...
export function LevelsChartDeepDiveLayout({
  ...
  const chartPane = (
    <>
      <div className="flex flex-1 min-h-0 min-w-0 mt-1 max-md:flex-none max-md:min-h-0">
        {toolbar}
        <div className="flex flex-col flex-1 min-h-0 min-w-0 overflow-hidden max-md:overflow-visible max-md:flex-none">
          {children}
        </div>
      </div>
    ...
    return (
      <div className={`flex flex-col flex-1 min-h-0 min-w-0 w-full ${className}`.trim()}>
        ...
        <div className="flex flex-1 min-h-0 min-w-0 mt-1.5 sm:mt-2">
          {symbolRailDesktop}
          <div className="flex flex-col flex-1 min-h-0 min-w-0 pl-[11px] sm:pl-[15px]">
