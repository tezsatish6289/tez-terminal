...
/** Shared config + AUTO/OFF trigger — identical on all four heatmap cards. */
export function BotCardToolbarTrigger({
  ...
  /** Shown beside gear on crypto (opens heatmap sheet). */
  sheetLabel?: string;
}) {
  ...
  return (
    <button
      type="button"
      className={cn(
        "flex items-center gap-1 shrink-0 rounded-lg border border-white/[0.12] bg-[#1a1a1f]",
    ...
    >
      ...
      <span
        className={cn(
          "flex items-center gap-0.5 px-1.5 py-0.5 rounded border text-[8px] font-black uppercase tracking-wider",
          autoColor,
        )}
      >
        {isForcedOff ? (
